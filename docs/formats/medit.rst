...
- On write, references come from ``element_attrs["ref"]``, failing that from the ``ref_<n>`` groups. A group named anything else has nowhere to go - a Medit record carries a number, not a name - so it is reported rather than numbered on the caller's behalf.
...
- A 2-D file's vertices are padded with a zero z, so the mesh is 3-D like every other one polyxios holds.
