...
def read(*, path: Source, lazy: bool = False) -> PolyData:
    ...
    PolyData
        ...
        only when at least one tag is non-zero.
...
def write(*, poly: PolyData, path: Source) -> None:
    ...
    if n_elems > 0:
        ...
        if np.any(kw_per_elem == -1):
            dropped = np.unique(type_codes[kw_per_elem == -1]).tolist()
            warnings.warn(
                f".meshb write: unsupported element type codes {dropped} dropped.",
                UserWarning,
                stacklevel=2,
            )
    else:
    ...
    with open_write(path) as fh:
        ...
        _write_i32(fh, _KW_DIMENSION)
        _write_i32(fh, dim)

        # Vertices section
        _write_i32(fh, _KW_VERTICES)
...
def _parse_header(mm: mmap.mmap | bytes) -> tuple[int, int, str]:
    ...
    if dim not in (2, 3):
        raise CodecError(f".meshb: unsupported dimension {dim}.")
...
def _scan_sections(
    ...
    while pos + 8 <= total:
        ...
        if count < 0:
            raise CodecError(
                f".meshb: negative section count {count} at file offset {pos}."
            )
        data_start = pos + 8
...
def _decode(mm: mmap.mmap | bytes) -> PolyData:
    ...
    elem_sizes_parts: list[np.ndarray] = []
    ...
    for kw, (elem_name, n_nodes) in _KW_TO_ELEM.items():
        ...
        arr = np.frombuffer(mm, dtype=elem_dt, count=n_elems, offset=estart)
