...
`--verbose` can be given on either side of the subcommand (e.g. `pxios --verbose fetch bunny.obj` or `pxios fetch bunny.obj --verbose`) to print debug logs and full tracebacks when a command fails. `pxios --version` prints the installed version and exits.
